 ...
 * }
    ...
    function arrayTuples() {
        ...
        }, 9007199254740991); // aka 2^53 鈭� 1 aka Number.MAX_SAFE_INTEGER
        var i, result = [];
...
// issue #2676
    ...
    /** @hidden */ var transLbl = function (trans) {
...
// TODO: explicitly make this user configurable
     ...
     * });
        ...
        Transition.prototype.toString = function () {
            ...
            var id = this.$id, from = isObject(fromStateOrName) ? fromStateOrName.name : fromStateOrName,
                fromParams = toJson(avoidEmptyHash(this._treeChanges.from.map(prop('paramValues')).reduce(mergeR, {}))),
                toValid = this.valid() ? "" : "(X) ", to = isObject(toStateOrName) ? toStateOrName.name : toStateOrName,
 ...
 * }
     ...
     * });
        ...
        StateRegistry.prototype._deregisterTree = function (state) {
            var _this = this;
            var all$$1 = this.get().map(function (s) {
                return s.$$state();
            });
            var getChildren = function (states) {
                var children = all$$1.filter(function (s) {
     ...
     * });
        ...
        UrlMatcher.prototype.format = function (values$$1) {
            ...
            var queryString = queryParams.map(function (paramDetails) {
                var param = paramDetails.param, squash = paramDetails.squash, encoded = paramDetails.encoded,
    ...
    defaultRuleSortFn = composeSort(sortBy(pipe(prop("priority"), function (x) {
        return -x;
    })), sortBy(pipe(prop("type"), function (type) {
        return ({"STATE": 4, "URLMATCHER": 4, "REGEXP": 3, "RAW": 2, "OTHER": 1})[type];
    })), function (a, b) {
        return (getMatcher(a) && getMatcher(b)) ? UrlMatcher.compare(getMatcher(a), getMatcher(b)) : 0;
    ...
    function addCoreResolvables(trans) {
        trans.addResolvable({
            token: UIRouter, deps: [], resolveFn: function () {
                return trans.router;
            }, data: trans.router
        }, "");
        trans.addResolvable({
 ...
 * }]
    ...
    function servicesPlugin(router) {
        ...
        services.$q = $q;
        return {
            name: "vanilla.services", $q: $q, $injector: $injector, dispose: function () {
                return null;
            }
        };
    }
    ...
    function ng1ViewsBuilder(state) {
        ...
        var tplKeys = ['templateProvider', 'templateUrl', 'template', 'notify', 'async'],
            ...
            compKeys = ['component', 'bindings', 'componentProvider'], nonCompKeys = tplKeys.concat(ctrlKeys),
    ...
    var Ng1LocationServices = (function () {
        ...
        Ng1LocationServices.monkeyPatchPathParameterType = function (router) {
            ...
            pathType.encode = function (val$$1) {
                return val$$1 != null ? val$$1.toString().replace(/(~|\/)/g, function (m) {
                    return ({'~': '~~', '/': '~2F'}[m]);
                }) : val$$1;
            };
            pathType.decode = function (val$$1) {
                return val$$1 != null ? val$$1.toString().replace(/(~~|~2F)/g, function (m) {
                    return ({'~~': '~', '~2F': '/'}[m]);
                }) : val$$1;
            };
        };
    ...
    function runBlock($injector$$1, $q$$1, $uiRouter) {
        ...
        $uiRouter.stateRegistry.get()
            ...
            .forEach(function (resolvable) {
                return resolvable.deps = $injector$$1.annotate(resolvable.resolveFn);
            });
    }
            ...
            var directive = {
                ...
                compile: function (tElement, tAttrs, $transclude) {
                    return function (scope, $element, attrs) {
                        var previousEl, currentEl, currentScope, unregister, onloadExp = attrs['onload'] || '',
                            autoScrollExp = attrs['autoscroll'], renderer = getRenderer(attrs, scope),
                            viewConfig = undefined, inherited = $element.inheritedData('$uiView') || rootData,
        ...
        if (isFunction(controllerInstance.uiOnParamsChanged)) {
            ...
            var paramsUpdated = function ($transition$) {
                ...
                if (changedToParams.length) {
                    ...
                    var newValues = filter(toParams, function (val$$1, key) {
